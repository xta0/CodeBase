...
function writeHeaderFile(n)
	local path = "./chap10_output.h"
	local f = assert(io.open(path, "w"))
	
	h = '// created by lua2objc \n\n'
	b = fwrite(f,h)

	h = "#import \"TBCitySBTableViewItem.h\"\n\n"
	b = fwrite(f,h)
	h = string.format("@interface %s : TBCitySBTableViewItem\n\n",n)
	b = fwrite(f,h)
	h="@end"
	b = fwrite(f,h)
